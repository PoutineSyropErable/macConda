...
class ResourceManager:
    ...
    def __load_resource(self, resource_type: ResourceType, file_path: Path) -> ImageTk.PhotoImage | ImageFont.FreeTypeFont | Path:
        ...
        if resource_type == ResourceType.IMAGE:
            ...
            return ImageFont.truetype(font=str(file_path), size=22)
        ...
        for rsrc_type in ResourceType:
            ...
            for file_path in resource_dir.rglob("*"):
                ...
                if file_path.stem.startswith('.'):
                    continue
                ResourceManager._cache[rsrc_type][str(relative_path)] = self.__load_resource(rsrc_type, file_path)

    def _get_resource_from_source(self, resource_type: ResourceType, name: str) -> bytes:
        data = open(f'resources/{resource_type.name.lower()}/{name}.{resource_type.extension}', 'rb').read()
        return data
    ...
    def __get_resource(self, resource_type: ResourceType, name: str) -> Any:
        ...
        # save to disk
        file_path = Path(f'rsrc_cache/{resource_type.name.lower()}/{name}.{resource_type.extension}')
    ...
    def render_font(self, font: ImageFont.FreeTypeFont, text: str, type_ : str = "normal") -> Image.Image:
        """
        ...
        image = Image.new(mode='RGB', size=(max_width, total_height), color=(255, 255, 255, 0))
        ...
        for idx, line in enumerate(lines):
            ...
            if type_ == "normal":
                ...
                draw.text((0, y_position), line, font=font, fill='black', stroke_width=1, stroke_fill='black')
            else:
...
class NetworkManager:
    ...
    def __init__(self, server_inbox: Queue, server_outbox: Queue, resource_manager: ResourceManager) -> None:
    ...
    def on_message(self, message: bytes) -> None:
        ...
        if data['classname'] == 'GridMessage':
            ...
            play_thread = threading.Thread(target=AudioPlayer.play_sound)
            play_thread.start()
        else:
...
class DialagueBox(Window):
    ...
    def __init__(self, root_window, resource_manager) -> None:
        ...
        super().__init__(root_window, title="Dialogue", width=GRID_WIDTH, height=100, offset_x=0, offset_y=-150)
    ...
    def start(self) -> None:
        ...
        if not self.__is_typing and len(self.__text_queue.queue) > 0:
            self.__current_text = self.__text_queue.get()
            self.__current_index = 0
    ...
    def __type_character(self) -> None:
        ...
        if self.__current_index < len(self.__current_text):
            ...
            image_text = self.__resource_manager.render_font(self.__text_font, self.__partial_text)
    ...
    def on_return(self, event) -> None:
        ...
        if not self.__is_typing and (len(self.__text_queue.queue) > 0 or self.__text_label.cget("image")):
...
def start() -> None:
    ...
    network_manager = NetworkManager(server_inbox, server_outbox, resource_manager)
